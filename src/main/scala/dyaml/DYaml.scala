...
object DYaml {
  ...
  def loadYAML(filename: String): SDynamic = {
    val yamlContent = Source.fromFile(filename).getLines.mkString
    DYamlString(StringContext(yamlContent)).dyaml0()
  }

  def loadYAML(inputStream: InputStream): SDynamic = {
    val yamlContent = Source.fromInputStream(inputStream).mkString
    DYamlString(StringContext(yamlContent)).dyaml0()
  }
